...
11px;padding-left: 5px"></a>
	...
	<div class="gwc-ft">
		<p id="tnum">
			all：${tnum}，total：<span style="color: #f60">￥${tprice }</span>
		</p>		
		<c:if test="${tnum==0}">
			<button onclick="showTip('Please add merchandise to cart！')">Pay</button>
		</c:if>
		<c:if test="${tnum!=0}">
	...
	<script type="text/javascript">
		...
		function min(goods_id, goods_price, sort) {
			...
			if (goods_num1 == 1 || goods_num1 < 1) {
				...
				//					}
				//			});
				var b = confirm('Are you sure delete this goods？');
				if (!b) {
					return;
				}
			}
