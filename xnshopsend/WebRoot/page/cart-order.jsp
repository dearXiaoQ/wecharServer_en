...
<body id="wrap">
	...
	<div class="sjsc-title2">
		...
		<a href="cartList.html" class="sjsc-t2r"><img
	...
	<dl class="drdd-info6"
		onclick="window.location.href='addrList.html?cps_id=${cps_id}&addr_id=${addr_id}'">
		<c:forEach items="${addr}" var="addr" begin="0" end="0">
			...
			<dt>
				<p>
					<span class="f-l">收货人：${addr.addr_user }</span> <span class="f-r">联系电话：${addr.addr_tel }</span>
				<div style="clear:both;"></div>
				</p>
				<p>
					收货地址：<span id="province" lang="${addr.province }"></span> <span
						id="city" lang="${addr.city }"></span> <span id="area"
						lang="${addr.area }"></span>${addr.addr_name }</p>
			</dt>
		</c:forEach>
	...
	<c:forEach items="${goods}" var="list" varStatus="s">
		...
		<div class="drdd-info3">
			...
			<h3 class="drdd-h31 f-l">
				<a href="#">${list.goods_name} x${list.goods_num}</a>
			</h3>
	...
	<c:if test="${tprice!=0}">
		...
		<div class="drdd-info4"
			onclick="window.location.href='cartCoupons.html?addr_id=${addr_id}'">
			<p>优惠券</p>
			<a> <c:forEach items="${cps}" var="cps">
	...
	<div class="drdd-info4">
		<p>配送方式</p>
		<a href="#"> <select id="fgt_price" onchange="fgt()"
			style="border: 0">
				...
				<option value="${fgt_price}" style="">免费自提点
					<%-- ：<c:forEach items="${userList}" var="userList">${userList.area_name}
	...
	<div
		style="font-size: 12px;padding-left:5px; margin-top:13px;color: #A09E9E;display:none;"
		id="zitidian-str">选择自提点</div>
	<div class="drdd-info2" style="display:none" id="zitidian-choose">
		<select id="area_area" style="border: 0;width: 20%"
			onchange="choose_area()">
			<option value="-2">请选择区</option>
			<c:forEach items="${areaList}" var="areaList">
				<option value="${areaList.area_id }">${areaList.area_name }</option>
			</c:forEach>
		</select> <select id="area_addr" style="border: 0;width:75%">
	...
	<div class="drdd-info4">
		...
		<input type="text" placeholder="选填，填写您对卖家的要求" id='note'
			style="width:88%;border:0px">
	...
	<div class="drdd-info2">
		...
		<div style="border-bottom:1px solid #DED9D9;">
			<c:forEach items="${cps}" var="cps">
				...
				<p class="p2 f-r">
					<span id="">￥${cps.cps_price}</span>
				</p>
				<br>
			</c:forEach>
		</div>
		<p class="p1 f-l">
			共<span id="tnumStr">${tnum }</span>件商品
		</p>
		<p class="p2 f-r">
			总计：<span id="tpriceStr" style="color: #f60">￥${tprice}</span>
		</p>
		<div style="clear:both;"></div>
	</div>
