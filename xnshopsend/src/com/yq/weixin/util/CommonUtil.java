...
public class CommonUtil
{
	...
	public String getJsapiTicket(String access_token){
		...
		System.out.println("-----------获取 ticket 开始 ");
		JSONObject jsonObject = httpsRequest(requestUrl, "GET", null);
		try
		{
			reStr = jsonObject.getString("ticket");
		}
		catch (JSONException e) {
			reStr = null;
			log.error("获取ticket失败 errcode:{} errmsg:{}", Integer.valueOf(jsonObject.getInt("errcode")), jsonObject.getString("errmsg"));
	...
	public Token getToken(String appid, String appsecret)
	{
		...
		if (add_time.longValue() + 7200000L <= nowtime.longValue()) {
			...
			try
			{
				...
				String jsapi_ticket = this.getJsapiTicket(jsonObject.getString("access_token"));
			...
			token.setJsapiTicket(((AccessToken)token_list.get(0)).getJsapi_ticket());
			System.out.println("-----------token:"+token.getAccessToken()+"'/n/r' jspiTicket:"+token.getJsapiTicket());
		}
		return token;
	}
